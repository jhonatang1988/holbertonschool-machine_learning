...
# linea 43 de las derivadas, sin aplicar la activacion porque tambien
# toca derivar por aparte la puta funcion de activacion

# linea 46 el layer previo OJO es el siguiente, pero vamos hacia el pasado.
def conv_backward(dZ, A_prev, W, b, padding="same", stride=(1, 1)):
    ...
    # los filtros tambien se derivan pero es mas como una convolution
    filtros = W
    ...
    if padding == 'same':
        altura_padding = int(np.ceil(((altura_layer_previa - 1) *
                                      altura_stride + altura_filtro -
    ...
    activaciones_con_padding = np.pad(
        layer_previa, ((0, 0), (altura_padding, altura_padding),
                       (largo_padding, largo_padding), (0, 0)), 'constant')
    ...
    derivadas_de_activaciones_con_padding = \
        np.pad(derivadas_de_activaciones, ((0, 0),
                                           (altura_padding, altura_padding),
                                           (largo_padding, largo_padding),
                                           (0, 0)))
    # toca una matrix para meter las derivadas de los filtros.
    derivadas_de_filtros = np.zeros(filtros.shape)
    # por ultimo la derivada de los biases
    derivadas_de_biases = \
        np.sum(derivadas_parciales_layer_sin_activacion, axis=(0, 1, 2),
               keepdims=True)
    # para cada imagen
